...
export const MultiSkillLevelChart = (props: { history: string[]; title: string; state: string; }) => {
    ...
    const getCharts = () => {
        ...
        fetch('https://localhost:44369/api/LeveTwo/multi-skill/' + proName, httpGetObject)
            .then(response => response.json())
            .then(data => {
                if (data != undefined) {
                    setChartData(data)
                    ...
                    const dataStruct = {
                        labels: labelsData,
                        data: entryData,
                        barColors: ["#3399FF", "#F17A17"],
                        legend: ["Single Skill", "Multi Skill"],
                    };
...
export default MultiSkillLevelChart;
MultiSkillLevelChart.defaultProps = {
    title: "Skill Base",
    searchBar: true
}
